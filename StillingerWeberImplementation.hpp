      ...
      for (int jj = 0; jj < numnei; ++jj)
      {
        ...
        if (rij_sq <= cutoffSq_2D_[iSpecies][jSpecies])
        {
          ...
          {  // effective half list
            ...
            {
              CalcPhiD2phiTwo(
                  iSpecies, jSpecies, rij_mag, phi_two, dphi_two, d2phi_two);
            ...
            else if ((isComputeProcess_dEdr == true)
                     || (isComputeForces == true) || (isComputeVirial == true)
                     || (isComputeParticleVirial == true))
            {
              CalcPhiDphiTwo(iSpecies, jSpecies, rij_mag, phi_two, dphi_two);
              if (particleContributing[j] == 1) { dEidr_two = dphi_two; }
              else
              {
                dEidr_two = HALF * dphi_two;
              }
            }
            else if ((isComputeEnergy == true)
                     || (isComputeParticleEnergy == true))
            {
            ...
            {
              if (particleContributing[j] == 1) { *energy += phi_two; }
              else
              {
                *energy += HALF * phi_two;
              }
            ...
            if (isComputeVirial == true)
            { ProcessVirialTerm(dEidr_two, rij_mag, rij, virial); }

            // Contribution to particleVirial
            if (isComputeParticleVirial == true)
            {
              ProcessParticleVirialTerm(
                  dEidr_two, rij_mag, rij, i, j, particleVirial);
            }
          ...
          for (int kk = jj + 1; kk < numnei; ++kk)
          {
            ...
            if (rik_sq <= cutoffSq_2D_[iSpecies][kSpecies])
            {
              ...
              // Contribution to energy
              if (isComputeEnergy == true) { *energy += phi_three; }
              ...
              if (isComputeVirial == true)
              {
                ProcessVirialTerm(dEidr_three[0], rij_mag, rij, virial);
                ProcessVirialTerm(dEidr_three[1], rik_mag, rik, virial);
                ProcessVirialTerm(dEidr_three[2], rjk_mag, rjk, virial);
              }
              ...
              {
                ProcessParticleVirialTerm(
                    dEidr_three[0], rij_mag, rij, i, j, particleVirial);
                ProcessParticleVirialTerm(
                    dEidr_three[1], rik_mag, rik, i, k, particleVirial);
                ProcessParticleVirialTerm(
                    dEidr_three[2], rjk_mag, rjk, j, k, particleVirial);
              }
